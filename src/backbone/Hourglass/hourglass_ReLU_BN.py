...
class fully_connected(nn.Module):
    ...
    def forward(self, x):
        ...
        bn = self.bn(relu) if self.with_bn else relu
        return bn
...
class exkp(nn.Module):
    ...
    modules:    [2, 2, 2, 2, 2, 4]
    heads:     heads = {'hm_t': 1, 'hm_l': 1,'hm_b': 1, 'hm_r': 1,}
    pre:
    cnv_dim:
    '''
    def __init__(
...
class HourglassNet_RB(exkp):
    def __init__(self, heads, num_stacks=1, num_branchs = 4, dims=[256, 256,256, 512, 1024] ):  # num_stacks: 沙漏结构数量 num_branchs: 子分支的数量
        ...
        super(HourglassNet_RB, self).__init__(
